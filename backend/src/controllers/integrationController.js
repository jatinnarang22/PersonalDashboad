  ...
  try {
    if (!yt.youtubeOAuthConfigured()) {
      return res.status(503).json({
        error:
          'YouTube OAuth is not configured. Set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI in backend/.env.',
      });
    }
...
export async function instagramStart(req, res) {
  try {
    if (!ig.instagramOAuthConfigured()) {
      return res.status(503).json({
        error:
          'Instagram OAuth is not configured. Set META_APP_ID, META_APP_SECRET, META_REDIRECT_URI in backend/.env.',
      });
    }
    const state = crypto.randomBytes(24).toString('hex');
...
export async function githubStart(req, res) {
  try {
    if (!gh.githubOAuthConfigured()) {
      return res.status(503).json({
        error:
          'GitHub OAuth is not configured. Set GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, GITHUB_REDIRECT_URI in backend/.env.',
      });
    }
    const state = crypto.randomBytes(24).toString('hex');
...
/** GET /api/integrations/github/activity — heatmap + event list (not heavily cached). */
export async function githubActivity(req, res, next) {
  try {
    ...
    if (!token) {
      return res.json({ connected: false, summary: null, contributionCalendar: null, recentEvents: [] });
    }
    const activity = await gh.fetchGitHubActivity(token);
    return res.json({ connected: true, ...activity });
  } catch (err) {
    next(err);
  }
}
